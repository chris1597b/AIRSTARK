      ...
      >
        <div className="flex justify-between items-start">
            <div className="flex-1">
                <span className={`text-[10px] font-bold tracking-[0.2em] uppercase ${mode === AppMode.QUIZ ? 'text-red-400' : 'text-teal-400'}`}>
                    {mode === AppMode.QUIZ ? 'EVALUACIÓN CLÍNICA' : 'FICHA CLÍNICA'}
                </span>
                <h2 className="text-2xl font-bold text-white mt-1 leading-tight truncate pr-2">
                    {mode === AppMode.QUIZ ? 'Caso Clínico' : selectedPart?.label}
                </h2>
            </div>
